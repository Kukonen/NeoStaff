...
internal class Program
{
    ...
    private async static Task Main(string[] args)
    {
        ...
        // Бесконечный цикл для ожидания завершения работы бота
    ...
    async static void SendDailyMessage(ITelegramBotClient botClient, List<long> chatIds)
    {
        ...
        Console.WriteLine(Server.GetLastSpecificationAsync().Result);
        ...
        {
            msg += "Сегодня желательно провести аттестацию у:\n";
    ...
    async static Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken token)
    {
        ...
        if (message.Text.Equals("/start"))
        {
            ...
            text: "Привет!\nДанный бот предназначен для предупреждения HR о необходимости проведения " +
                ...
                "месяц не проходили аттестацию или набрали необходимое количество баллов для повышения!",
                null);
        ...
        else
        {
            Message sentMessage = await botClient.SendTextMessageAsync(
                ...
                "/start - чтобы начать получать оповещения об аттестациях!",
                null);
        }
    }
